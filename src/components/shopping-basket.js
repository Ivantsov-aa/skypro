...
import {
  ...
  incrementCount,
  removeFromCart,
} from "../../redux/slices/shoppingBasketSlice";
import CallbackForm from "./basket/callback-form";
...
export default function ShoppingBasket() {
  ...
  return (
    <main className="shopping-basket">
      <section className="shopping-basket__products">
        <div className="shopping-basket__products-header">
          <p>Товар</p>
          <p>К-во</p>
        </div>
        <div className="shopping-basket__products-container">
          {!products.length && (
            <div className="empty-plug">
              <SvgEmptyCart />
              <h3>Ваша корзина пуста</h3>
              <p>
                Перейдите в <Link to="/">каталог</Link> и добавьте товары.
              </p>
            </div>
          )}
          {products.map((product) => (
            <div className="shopping-basket__product" key={product.id}>
              <div className="info">
              ...
              <div className="counter-buttons">
                ...
                <div className="buttons">
                  <button onClick={() => dispatch(incrementCount(product))}>
                    <SvgArrowIcon />
                  </button>
                  <button onClick={() => dispatch(decrementCount(product))}>
                    <SvgArrowIcon />
                  </button>
                </div>
              </div>
        ...
        <div className="shopping-basket__products-btns">
          <button className="outline" onClick={() => dispatch(clearBasket())}>
            Очистить корзину
            <TouchEffect />
          </button>
      ...
      <CallbackForm onSubmit={setSubmitForm} />
      {submitForm && (
        <NotificationModal
          {...submitForm}
          onClose={() => {
...
const SvgEmptyCart = () => {
  return (
    <svg fill="#000" height="100px" width="100px" viewBox="0 0 231.523 231.523">
      <g>
        <path
