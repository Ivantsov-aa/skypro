...
import { addToCart } from "../../../redux/slices/shoppingBasketSlice";

export default function CatalogCart({ cart }) {
  const dispatch = useDispatch();
  const { preview, title, text, price, path } = cart;
    ...
    <div className="catalog__cart">
      ...
      <Link to={path || "/"}>
        <div className="preview">
          <img src={preview} alt="skypro" />
        </div>
        <div className="description">
          <h2 className="title">{title}</h2>
          <p className="text">{text}</p>
          <p className="price">{price.toLocaleString("ru-RU")} руб.</p>
        </div>
      </Link>
    </div>
  );
